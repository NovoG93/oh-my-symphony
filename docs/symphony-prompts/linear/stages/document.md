...
4. Append `## Learnings` -- 3-4 bullets future work should not rediscover.
5. Append `## Wiki Updates` -- paths created/modified/removed, one line each.
6. Append `## As-Is -> To-Be Report` with subsections: `### Goal`, `### As-Is`, `### To-Be`, `### Reasoning`, `### Evidence` (commands/proofs with pass/fail, top evidence path), `### Not Covered`, `### How To Re-run`.
7. Only if a real critical/manual intervention remains, append `## Human Review` with: `### Intervention Required`, `### What Changed`, `### Why It Matters`, `### Evidence`, `### Risks`, `### Human Checklist` (3-5 quickly verifiable checkboxes), `### Decision Needed` (exactly one line: `Confirm Done` or `Do not confirm; move back to <state> because <reason>`).
