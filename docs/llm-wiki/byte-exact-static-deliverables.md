...
4. **Prove the merged tree** — when `git merge-tree` is denied, `git merge-base
   develop HEAD` == develop tip proves zero conflict surface: the merged tree
   is exactly the branch tree, so the post-merge `ls-files` check holds by
   construction.
